...
if (isset($_GET['query'])) {
    // echo "raw data from input: " . $_GET['query'];
    ...
    if (empty($result)) {
        echo "<h1>SUPERHERO NOT FOUND</h1>";
    } else {
        foreach ($result as $hero) {
            $upperAlias = strtoupper($hero['alias']);
            $upperName = strtoupper($hero['name']);

            echo "<h3>{$upperAlias}</h3>";
            echo "<h4>{$upperName}</h4>";
